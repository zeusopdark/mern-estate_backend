...
export const google = async (req, res, next) => {
    try {
        const user = await User.findOne({ email: req.body.information.email });
        ...
        if (user) {
            ...
            const newUser = new User({
                username: (req.body.information.name.split(" ").join("").toLowerCase() +
                    Math.random().toString(36).slice(-4)),
                email: req.body.information.email,
                password: hashedPassword,
                avatar: req.body.information.photo
            });
            console.log(req.body.information.photo);
            await newUser.save();
